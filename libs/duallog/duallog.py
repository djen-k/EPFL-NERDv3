...
def setup(logdir='log', logname='log', minlevelConsole=logging.WARNING, minLevelFile=logging.DEBUG, lformat=None):
    ...
    # Logfile name format
    if lformat is None:
        lformat = '{name}-{year:04d}{mon:02d}{day:02d}-{hour:02d}{min:02d}{sec:02d}.log'

    # Construct the logfile name.
    t = datetime.datetime.now()
    logfile = lformat.format(year=t.year, mon=t.month, day=t.day, hour=t.hour, min=t.minute, sec=t.second, name=logname)
